...
bool BattlefieldTB::SetupBattlefield()
{
    ...
    InitStalker(NPC_DEBUG_ANNOUNCER, TolBaradDebugAnnouncerPos[0], TolBaradDebugAnnouncerPos[1], TolBaradDebugAnnouncerPos[2], TolBaradDebugAnnouncerPos[3]);

    m_MaxPlayer = sWorld->getIntConfig(WorldIntConfigs::CONFIG_TOLBARAD_PLR_MAX);
    m_IsEnabled = sWorld->getBoolConfig(WorldBoolConfigs::CONFIG_TOLBARAD_ENABLE);
    m_MinPlayer = sWorld->getIntConfig(WorldIntConfigs::CONFIG_TOLBARAD_PLR_MIN);
    m_MinLevel = sWorld->getIntConfig(WorldIntConfigs::CONFIG_TOLBARAD_PLR_MIN_LVL);
    m_BattleTime = sWorld->getIntConfig(WorldIntConfigs::CONFIG_TOLBARAD_BATTLETIME) * MINUTE * IN_MILLISECONDS;
    m_BonusTime = sWorld->getIntConfig(WorldIntConfigs::CONFIG_TOLBARAD_BONUSTIME) * MINUTE * IN_MILLISECONDS;
    m_NoWarBattleTime = sWorld->getIntConfig(WorldIntConfigs::CONFIG_TOLBARAD_NOBATTLETIME) * MINUTE * IN_MILLISECONDS;
    m_RestartAfterCrash = sWorld->getIntConfig(WorldIntConfigs::CONFIG_TOLBARAD_RESTART_AFTER_CRASH) * MINUTE * IN_MILLISECONDS;
...
// 62 fields, [7]-[68]
void BattlefieldTB::FillInitialWorldStates(WorldStateBuilder& builder)
{
    builder.AppendState(uint32(TB_WS_ALLIANCE_ATTACKING_SHOW), int32(IsWarTime() && GetAttackerTeam() == TEAM_ALLIANCE ? 1 : 0));
    builder.AppendState(uint32(TB_WS_HORDE_ATTACKING_SHOW), int32(IsWarTime() && GetAttackerTeam() == TEAM_HORDE ? 1 : 0));

    // Not sure if TB
    //builder.AppendState(uint32(TB_WS_9_UNKNOWN), int32(1));

    builder.AppendState(uint32(TB_WS_SOUTH_DAMAGED_NEUTRAL), int32(0));
    builder.AppendState(uint32(TB_WS_SOUTH_INTACT_NEUTRAL), int32(0));

    builder.AppendState(uint32(TB_WS_PROGRESS_SHOW), int32(0));
    ...
    for (BfCapturePointMap::iterator itr = m_capturePoints.begin(); itr != m_capturePoints.end(); ++itr)
    {
        ...
        {
            case BattlefieldObjectiveStates::BF_CAPTUREPOINT_OBJECTIVESTATE_ALLIANCE:
            case BattlefieldObjectiveStates::BF_CAPTUREPOINT_OBJECTIVESTATE_HORDE:
                controlled = true;
                team = itr->second->GetTeamId();
                break;
            case BattlefieldObjectiveStates::BF_CAPTUREPOINT_OBJECTIVESTATE_HORDE_ALLIANCE_CHALLENGE:
            case BattlefieldObjectiveStates::BF_CAPTUREPOINT_OBJECTIVESTATE_NEUTRAL_ALLIANCE_CHALLENGE:
                team = TEAM_ALLIANCE;
                capturing = true;
                break;
            case BattlefieldObjectiveStates::BF_CAPTUREPOINT_OBJECTIVESTATE_ALLIANCE_HORDE_CHALLENGE:
            case BattlefieldObjectiveStates::BF_CAPTUREPOINT_OBJECTIVESTATE_NEUTRAL_HORDE_CHALLENGE:
                team = TEAM_HORDE;
                capturing = true;
                break;
            default:
        ...
        builder.AppendState(uint32(TBCapturePoints[i].wsControlled[TEAM_ALLIANCE]), int32(team == TEAM_ALLIANCE && controlled ? 1 : 0));
        builder.AppendState(uint32(TBCapturePoints[i].wsCapturing[TEAM_ALLIANCE]), int32(team == TEAM_ALLIANCE && capturing ? 1 : 0));
        builder.AppendState(uint32(TBCapturePoints[i].wsNeutral), int32(team == TEAM_NEUTRAL ? 1 : 0));
        builder.AppendState(uint32(TBCapturePoints[i].wsCapturing[TEAM_HORDE]), int32(team == TEAM_HORDE && capturing ? 1 : 0));
        builder.AppendState(uint32(TBCapturePoints[i].wsControlled[TEAM_HORDE]), int32(team == TEAM_HORDE && controlled ? 1 : 0));
    }

    builder.AppendState(uint32(TB_WS_TOWERS_DESTROYED_SHOW), int32(GetData(BATTLEFIELD_TB_DATA_TOWERS_DESTROYED)));
    builder.AppendState(uint32(TB_WS_BUILDINGS_CAPTURED_SHOW), int32(IsWarTime() ? 1 : 0));
    builder.AppendState(uint32(TB_WS_BUILDINGS_CAPTURED), int32(GetData(BATTLEFIELD_TB_DATA_BUILDINGS_CAPTURED)));
    builder.AppendState(uint32(TB_WS_TOWERS_DESTROYED), int32(0));
    ...
    // Unused tower icons
    builder.AppendState(uint32(TB_WS_WEST_DAMAGED_NEUTRAL), int32(0));
    builder.AppendState(uint32(TB_WS_WEST_INTACT_NEUTRAL), int32(0));
    builder.AppendState(uint32(TB_WS_EAST_DAMAGED_NEUTRAL), int32(0));
    builder.AppendState(uint32(TB_WS_EAST_INTACT_NEUTRAL), int32(0));
    ...
    for (uint8 i = 0; i < TB_TOWERS_COUNT; i++)
    {
        ...
        for (ObjectGuid guid : Towers)
        {
            ...
            builder.AppendState(uint32(TBTowers[i].wsIntact[TEAM_ALLIANCE]), int32(!team && intact ? 1 : 0));
            builder.AppendState(uint32(TBTowers[i].wsDamaged[TEAM_ALLIANCE]), int32(!team && damaged ? 1 : 0));
            builder.AppendState(uint32(TBTowers[i].wsDestroyed), int32(destroyed ? 1 : 0));
            builder.AppendState(uint32(TBTowers[i].wsDamaged[TEAM_HORDE]), int32(team && damaged ? 1 : 0));
            builder.AppendState(uint32(TBTowers[i].wsIntact[TEAM_HORDE]), int32(team && intact ? 1 : 0));
        }
    }

    builder.AppendState(uint32(TB_WS_TIME_NEXT_BATTLE_SHOW), int32(!IsWarTime() ? 1 : 0));

    builder.AppendState(uint32(TB_WS_ALLIANCE_CONTROLS_SHOW), int32(!IsWarTime() && GetDefenderTeam() == TEAM_ALLIANCE ? 1 : 0));
    builder.AppendState(uint32(TB_WS_HORDE_CONTROLS_SHOW), int32(!IsWarTime() && GetDefenderTeam() == TEAM_HORDE ? 1 : 0));

    builder.AppendState(uint32(TB_WS_TIME_BATTLE_END), int32(IsWarTime() ? time(NULL) + (m_Timer / 1000) : 0));
    builder.AppendState(uint32(TB_WS_TIME_NEXT_BATTLE), int32(!IsWarTime() ? time(NULL) + (m_Timer / 1000) : 0));

    // Not sure if TB
    //builder.AppendState(uint32(TB_WS_65_UNKNOWN), int32(0));
    //builder.AppendState(uint32(TB_WS_66_UNKNOWN), int32(0));

    builder.AppendState(uint32(TB_WS_KEEP_ALLIANCE), int32(GetDefenderTeam() == TEAM_ALLIANCE ? 1 : 0));
    builder.AppendState(uint32(TB_WS_KEEP_HORDE), int32(GetDefenderTeam() == TEAM_HORDE ? 1 : 0));    
}
...
void BattlefieldTB::UpdateNPCsAndGameObjects()
{
    ...
    if (GameObject* door = GetGameObject(m_gateToTheHoleGUID))
        door->SetGoState(m_iCellblockRandom == CELLBLOCK_THE_HOLE ? GOState::GO_STATE_ACTIVE : GOState::GO_STATE_READY);

    // D-Block gate
    if (GameObject* door = GetGameObject(m_gateDBlockGUID))
        door->SetGoState(m_iCellblockRandom == CELLBLOCK_D_BLOCK ? GOState::GO_STATE_ACTIVE : GOState::GO_STATE_READY);
...
void BattlefieldTB::OnGameObjectCreate(GameObject* go)
{
    switch (go->GetEntry())
    {
        ...
        case GO_GATE_TO_THE_HOLE:
            ...
            go->SetGoState(m_iCellblockRandom == CELLBLOCK_THE_HOLE ? GOState::GO_STATE_ACTIVE : GOState::GO_STATE_READY);
            break;
        case GO_GATE_D_BLOCK:
            m_gateDBlockGUID = go->GetGUID();
            go->SetGoState(m_iCellblockRandom == CELLBLOCK_D_BLOCK ? GOState::GO_STATE_ACTIVE : GOState::GO_STATE_READY);
            break;
        case GO_CURSED_DEPTHS_GATE:
            m_gateCursedDepthsGUID = go->GetGUID();
            go->SetGoState(m_iCellblockRandom == CELLBLOCK_CURSED_DEPTHS ? GOState::GO_STATE_ACTIVE : GOState::GO_STATE_READY);
            break;
...
{
    if (killer == victim || victim->GetTypeId() != TypeID::TYPEID_PLAYER)
        return;
...
TolBaradCapturePoint::TolBaradCapturePoint(BattlefieldTB* battlefield, TeamId teamInControl) : BfCapturePoint(battlefield)
{
    ...
    m_State = teamInControl == TEAM_ALLIANCE ? BattlefieldObjectiveStates::BF_CAPTUREPOINT_OBJECTIVESTATE_ALLIANCE : BattlefieldObjectiveStates::BF_CAPTUREPOINT_OBJECTIVESTATE_HORDE;
}
